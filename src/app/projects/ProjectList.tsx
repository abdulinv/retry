...
function ProjectList({ projectList }: ProjectListProps) {
  ...
  const filteredList = projectList.filter(
    (item) => item.doc.tags?.includes(tag) || tag === ''
  );
  const searchedProjectList = filteredList.filter((item) =>
    item.doc.title?.includes(search)
  );
  ...
  return (
    ...
    >
      <Stack
        sx={{ backgroundColor: 'whitesmoke', mx: 20, borderRadius: 2 }}
        mb={2}
        gap={2}
        flexDirection={'row'}
            ...
            <Grid2 key={item.id} size={3}>
              <Card>
                ...
                >
                  ...
                  {edit.field !== item.doc.description &&
                    edit.id !== item.id && (
                      <Box
                        component={'div'}
                        onDoubleClick={
